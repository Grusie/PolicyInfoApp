...
class SignUpViewModel @Inject constructor(
    ...
    fun signUpEmail() {
        when {
            _idText.value.isEmpty() || _pwText.value.isEmpty() -> { //필수값이 입력되지 않았을 때
                setSignUpEventState(SignUpEventState.Alert(Constant.ERROR_CODE_EMPTY))
            }
            ...
            !isValidPassword() -> { //비밀번호 패턴이 맞지 않을 때
                setSignUpEventState(SignUpEventState.Alert(Constant.ERROR_PASSWORD_FORMAT))
            }
                ...
                viewModelScope.launch {
                    try {
                        ...
                        setSignUpUiState(SignUpUiState.Init)
                        setSignUpEventState(SignUpEventState.Error(e))
                    }
                }
            }
        }
    ...
    fun sendEmail() {
        if (_idText.value.trim().isEmpty()) {
            ...
            setSignUpEventState(SignUpEventState.Alert(Constant.ERROR_CODE_FORMAT))
        } else {
            viewModelScope.launch {
                setSignUpUiState(SignUpUiState.Loading)
                try {
    ...
    private fun setSignUpEventState(state: SignUpEventState) {
        viewModelScope.launch {
            _signUpEventState.emit(state)
        }
    }
}